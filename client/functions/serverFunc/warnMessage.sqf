...
{
	_message = format ["Admin: %1", _warnText];		// I (Krycek) want to hide who the calling Admin is. Here is the original: _message = format ["Admin %2: %1", _warnText, _adminName]; <- Displays the admins name to the player being warned. This makes it impossible to hide your identity as Admin. Which could be good since, I at least, log in with a different player name so I don't spoke any potential Hackers.
